...
uniform vec3 light = normalize(vec3(-1.0, 0.0, -1.0));
...
void main(void) {
		vec3 normalMap = (2.0 * texture(cubeNormMap, ex_UVW).rgb - 1.0);
		...
		out_Color = vec4(vec3(texture(cubeMap, ex_UVW)*cosTheta), 1.0f);
}
